...
## ADR-032 · 배포는 프로젝트마다 복붙하지 않고 npm 패키지로 한다

상태: Accepted 확신도: 높음 날짜: 2026-09-09

### 맥락

지금까지 SI 프로젝트는 이 저장소의 SCSS 와 sprite 를 **복사해서** 썼다.
그래서 프로젝트마다 사본이 생기고, 사본은 고쳐지지 않는다. 어느 프로젝트가 어느 시점의
디자인시스템을 쓰고 있는지 알 방법도 없다.

### 결정

**npm 패키지로 배포하고 프로젝트는 설치해서 쓴다.** 사내 시스템과 앞으로 추가되는
SI 프로젝트 전부가 대상이다. 소비하는 쪽에 Node 빌드가 없으면 추가한다.

레지스트리는 GitHub Packages 다. 공개 레지스트리는 한 번 올리면 되돌릴 수 없어 쓰지 않는다.

### 코드 작업에 무엇이 달라지나

- **주 산출물은 컴파일된 CSS 다.** SCSS 원본도 함께 배포하지만, 그건 프로젝트가 새 컴포넌트를
  만들 때 믹스인을 쓰기 위한 것이다. 값을 컴파일 시점에 덮어쓰라는 뜻이 아니다
- **아이콘 sprite 와 로고도 배포물이다.** 지금 프로젝트가 복사해 가는 것들이다
- **클래스 이름과 토큰 이름이 공개 API 가 된다.** 지우거나 바꾸면 메이저 버전이고
  설치한 프로젝트를 전부 고쳐야 한다. 첫 배포 전에 정리해야 하는 이유다
- 퍼블리시 사슬 끝에 한 칸이 붙는다 — 코드 반영 다음에 **패키지 배포**, 그다음 프로젝트가 설치

### 아직 안 정한 것

- 저장소를 조직 계정으로 옮길지 (패키지 이름의 스코프가 소유 계정 이름이라 첫 배포 전에만 바꿀 수 있다)
- 패키지 형태와 배포 자동화

**착수 조건이 아직 아니다.** Figma 변수가 계속 바뀌는 중이고 값 스냅샷이 낡았다.
계획과 착수 순서는 Cowork 프로젝트 `claude/package-design.md` 에 있다 — 코드 세션에서는
읽을 수 없으므로, 그 단계가 오면 이 ADR 을 갱신한다.
...
**첫 배포 전에만 가능하다.** 배포 후에는 클래스와 토큰 이름이 공개 API 이고(ADR-032),
...
별칭 유예 기간이 필요하다 — ADR-032 의 이름 변경 절차를 따른다.
